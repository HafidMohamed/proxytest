...
def pre_issue_checks(domain: str) -> Tuple[bool, str]:
    ...
    # BUG FIX: Original did HTTP round-trip with Host: domain header, which
    # fails when the domain vhost isn't loaded yet (default_server returns 444).
    # Instead: check nginx is listening on port 80, then verify the file is
    # readable from disk. certbot does the real HTTP check itself.
    import socket
    try:
    ...
    try:
        if fpath.read_text().strip() != "ok":
            return False, f"ACME webroot write/read mismatch at {fpath}"
    except Exception as exc:
        return False, f"Cannot read back ACME test file: {exc}"
    finally:
        try:
            fpath.unlink()
        except Exception:
    ...
    return True, "Pre-checks passed: nginx running and ACME webroot is writable"
